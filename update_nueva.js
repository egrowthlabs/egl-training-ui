...
function updateNuevaPage() {
  const file = 'app/dashboard/workouts/nueva/page.tsx';
  ...
  // Remove default variables from state
  content = content.replace(
    /defaultRounds:\s*\d+,[\s\S]*?defaultRestTimerSeconds:\s*\d+,/,
    ''
  );

  // Add block state variables
  const blockState = `
...
`;
  ...
  // Add Blocks UI before submit button
  const blocksUI = `
        {/* Bloques */}
                ...
                {block.exercises.map((ex, ei) => (
                  <div key={ei} className="flex flex-wrap md:flex-nowrap items-center gap-3 bg-secondary/20 rounded-lg p-3">
                    ...
                    {(ex.trackingType === 'reps' || ex.trackingType === 'reps_weight') && (
                      <input type="number" placeholder="reps" value={ex.overrideReps ?? ''}
                    ...
                    {(ex.trackingType === 'time' || ex.trackingType === 'time_weight') && (
                      <input type="number" placeholder="seg" value={ex.overrideDurationSeconds ?? ''}
